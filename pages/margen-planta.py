        ...
        [
            ...
            html.Div(
                [
                    html.Div([
                        'Seleccione empresa',
                        dcc.Dropdown(id='Emp2', options=emp_dict.loc['Empresa'].unique(), value=[emp_dict.loc['Empresa'].unique()[0]], clearable=False, multi=True), # Selección de la simulación
                    ], className='no-print'),
                    html.Br([]),
                    html.Div([
                        'Seleccione caso',
                    ...
                    html.Div([
                        'Seleccione central',
                        dcc.Dropdown(id = 'Cen2', value=auxCBcen_df['cen_nom'].unique()[0]), # Selección de la central (por nombre)
                    ], className='no-print'),
                    html.Br([]),
                    html.Div([
...
def update_planta(caso,Cen,Sim,init,end):
    ...
    for id, i in enumerate(caso): # Se recorren lo plp si hay más de 1
        ...
        fig2.add_trace(
            go.Bar(x=df1['date'], y=df1['CenCVar'], name="Costo variable " + i +  " [USD|MW]")
        )

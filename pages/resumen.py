...
#2.0
dash.register_page(__name__,path='/resumen')
        ...
        [
            ...
            html.Div(
                [
                    html.Div([
                        'Seleccione empresa',
                        dcc.Dropdown(id='Emp1', options=emp_dict.loc['Empresa'].unique(), value=emp_dict.loc['Empresa'].unique()[0], clearable=False), # Selección de la simulación
                    ], className='no-print'),
                    html.Br([]),
                    html.Div([
                        'Seleccione caso',
                        dcc.Dropdown(id='Caso1', options=['Caso base', 'Caso falla CTA engie'], value='Caso base', clearable=False), # Selección de la simulación
                    ], className='no-print'),
                    html.Br([]),
                    html.Div([
                        'Seleccione central',
                        dcc.Dropdown(id = 'Cen1', value=cen_df['cen_nom'].unique()[0]), # Selección de la central (por nombre)
                    ], className='no-print'),
                    html.Br([]),
                    html.Div([
                        'Seleccione simulación',
                        dcc.Dropdown(id='Sim1', options=cen_df['Hidro'].unique(), value=cen_df['Hidro'].unique()[0], clearable=False), # Selección de la simulación
                    ], className='no-print'),
                    # Row 3
                    html.Br([]),
                    html.Br([]),
...
@callback(
    [Output(component_id='margen_planta1', component_property='figure'),
     Output(component_id='c_var1', component_property='figure'),
...
def update_planta(caso,Cen, Sim):
    ...
    elif caso == 'Caso falla CTA engie':
        df1 = cenF_df.loc[cenF_df['cen_nom'].str.strip() == Cen.strip()] # Se filtra por nombre de la central
        df1 = df1.loc[df1['Hidro'].str.strip() == Sim.strip()]
        bar = df1['bar_nom'].unique()[0].strip()
        df2 = barF_df.loc[barF_df['bar_nom'].str.strip() == bar]
        df2 = df2.loc[df2['Hidro'].str.strip() == Sim.strip()]
        df1 = df1.sort_values(by=['date']) # Se ordenan los datos por fecha para los gráficos de linea
        df2 = df2.sort_values(by=['date']) # Se ordenan los datos por fecha para los gráficos de linea
    plpdate="2023.03"
